    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
        <div className="flex items-center gap-3 mb-6">
          <div
            className="w-10 h-10 rounded-xl flex items-center justify-center bg-purple-50"
          >
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">Monto transaccionado según medio de pago</p>
          </div>
        </div>
        ...
        {pieData.length > 0 ? (
          <div className="flex flex-col md:flex-row items-center justify-center gap-8">
            <div className="w-full max-w-[300px] h-[300px]">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <RechartsTooltip formatter={(value: number) => ["$" + value.toFixed(2), "Total"]} />
            ...
            <div className="flex-1 w-full flex flex-col gap-3">
              {pieData.sort((a,b) => b.value - a.value).map((entry, idx) => {
                const percentage = totalVentas > 0 ? (entry.value / totalVentas * 100).toFixed(1) : 0;
                return (
                  <div key={idx} className="flex items-center justify-between p-3 rounded-lg border border-gray-50 hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-3">
                      <div className="w-3 h-3 rounded-full" style={{ backgroundColor: COLORS[idx % COLORS.length] }} />
                      <span className="font-semibold text-gray-700 text-sm">{entry.name}</span>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-gray-900 text-sm">${entry.value.toFixed(2)}</p>
          ...
          <div className="text-center py-12">
            <p className="text-gray-500 text-sm">Aún no hay ventas para mostrar esta estadística.</p>
          </div>
        )}
      </div>
