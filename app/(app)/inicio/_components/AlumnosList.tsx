...
function NuevoAlumnoModal({ onClose, onGuardado }: { onClose: () => void; onGuardado: () => void }) {
  ...
  const [planesSuscripcion, setPlanesSuscripcion] = useState<Array<{ name: string; duration_days: number; price: number }>>([]);
  ...
  function validatePaso1(): Record<string, string> {
    ...
    if (!form.fechaRegistro) e.fechaRegistro = "La fecha de registro es requerida";
  ...
  async function handleGuardarPaso2() {
    ...
    const planElegido = planesSuscripcion.find((p) => p.name === pagoForm.actividad);
    ...
    const { error: errorUpdate } = await supabase.from("alumnos").update({
  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-4 md:px-6 py-4 border-b border-gray-100 shrink-0">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              {step === 1 ? "Completá los datos personales." : "Asigná una actividad y registra el pago."}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Panel Paso 1 */}
        {step === 1 && (
          <form onSubmit={(e) => { e.preventDefault(); handleGuardarPaso1(false); }} className="px-4 md:px-6 py-5 flex flex-col gap-4 overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Nacimiento *</label>
                <input
                  type="date"
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Registro *</label>
                <input
                  type="date"
                  ...
                  className="border border-gray-200 rounded-lg px-4 py-3 text-sm outline-none focus:border-red-400 focus:ring-2 focus:ring-red-50"
                />
                {errors.fechaRegistro && (<span className="text-xs text-red-500">{errors.fechaRegistro}</span>)}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Género *</label>
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                className="border border-gray-200 rounded-lg px-4 py-3 text-sm outline-none focus:border-red-400 focus:ring-2 focus:ring-red-50"
              />
              {errors.domicilio && <span className="text-xs text-red-500">{errors.domicilio}</span>}
            </div>
        ...
        {/* Panel Paso 2 */}
        {step === 2 && (
          <form onSubmit={(e) => { e.preventDefault(); handleGuardarPaso2(); }} className="px-4 md:px-6 py-5 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Actividad *</label>
              <div className="relative">
                <select
              ...
              {errors.actividad && <span className="text-xs text-red-500">{errors.actividad}</span>}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Medio de Pago *</label>
                <div className="relative">
                  <select
                ...
                {errors.medioPago && <span className="text-xs text-red-500">{errors.medioPago}</span>}
              </div>

              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Fecha de Cobro *</label>
                <input
                  type="date"
                ...
                />
                {errors.fechaCobro && <span className="text-xs text-red-500">{errors.fechaCobro}</span>}
              </div>
            </div>
        ...
        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1">
          ...
          {alumno.fecha_ultima_asistencia && (
            <span className="inline-flex items-center gap-1 text-xs font-medium text-red-600 bg-red-50 px-1.5 py-0.5 rounded">
              U. Asistencia: {formatFecha(alumno.fecha_ultima_asistencia)}
            </span>
          )}
          {alumno.dni && (
  ...
  return (
    <div className="p-4 md:p-6 lg:p-8 max-w-[1600px] mx-auto">
      ...
      <p className="text-xs text-gray-400 mb-3 font-medium select-none">
        {queryActual ? (
          <>
            <span className="text-gray-600 font-semibold">{totalRegistros}</span> resultado
            {totalRegistros !== 1 ? "s" : ""} para{" "}
            <span className="text-gray-600 font-semibold">&ldquo;{queryActual}&rdquo;</span>
          </>
        ) : (
          <>
      ...
      {alumnosEnriquecidos.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-gray-500 font-medium text-base">No se encontraron alumnos</p>
